...
export default function App() {
  ...
  return (
    <ThemeContext.Provider value={{ dark, toggle }}>
      <BrowserRouter><AppInner dark={dark} /></BrowserRouter>
...
function AppInner({ dark }) {
  ...
  return (
    /*
      ⚠️ TRANSLATION FIX:
      Chrome's "Translate page" injects a <google-translate-element> into the DOM
      and sets overflow:hidden on <html>, causing a black screen.
      Fix: Never set overflow:hidden on html/body. Use overflowX:hidden on the wrapper div only.
      Also avoid `cursor:none` on html — only apply it via the CursorSystem component's <style>.
    */
    <div
      ...
      <style>{`
        /* ── TRANSLATION FIX: never block overflow on html/body ──
           Chrome translate injects elements into <body> which can cause
           layout issues if overflow is hidden at the html level. */
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* ── TRANSLATION FIX: keep text readable when Google Translate replaces spans ── */
        [lang], font { color:inherit !important; }

        ::selection { background:#00ff88; color:#040608; }
        div::-webkit-scrollbar { display:none; }

        /* ── POWER4.OUT EASING KEYFRAMES ── */
        ...
        /* Mobile glow pulse for CTAs */
        @keyframes mobilePulse{0%,100%{box-shadow:0 0 14px rgba(0,255,136,.3),0 4px 18px rgba(0,255,136,.2);}50%{box-shadow:0 0 28px rgba(0,255,136,.55),0 6px 28px rgba(0,255,136,.4);}}

        /* ── BUTTONS — Power4 easing + shine sweep ── */
        .btn-g{
          ...
          /* Mobile: glow pulse instead of hover */
