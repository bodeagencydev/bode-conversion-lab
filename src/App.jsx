...
function AppInner({ dark }) {
  ...
  /* ── GOLDEN LIGHT THEME — deep liquid gold from reference image ── */
  const bg = dark ? "#040608" : "#C8960C";
  const fg = dark ? "#f0f0f0" : "#1A1000";

  /* ── TRUE NEON GREEN — matches the bright center glow in reference ── */
  const G_LIGHT = "#00FF88";
  const G_DARK  = "#00FF88";
    ...
    <div
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        body {
          overflow-x:hidden;
          background:${dark ? "#040608" : "linear-gradient(160deg, #B8860B 0%, #D4A017 25%, #F5C518 50%, #D4A017 75%, #B8860B 100%)"} !important;
        }
        ...
        @keyframes glowPulse{0%,100%{box-shadow:0 0 18px rgba(0,255,136,.25),0 4px 22px rgba(0,255,136,.35);}50%{box-shadow:0 0 50px rgba(0,255,136,.65),0 8px 50px rgba(0,255,136,.65);}}
        @keyframes mobilePulse{0%,100%{box-shadow:0 0 18px rgba(0,255,136,.4);}50%{box-shadow:0 0 36px rgba(0,255,136,.8);}}
        ...
        @keyframes glow{0%,100%{box-shadow:0 0 24px rgba(0,255,136,.3);}50%{box-shadow:0 0 55px rgba(0,255,136,.7);}}
        ...
        @keyframes greenGlow{0%,100%{box-shadow:0 0 20px rgba(0,255,136,.3);}50%{box-shadow:0 0 60px rgba(0,255,136,.9),0 0 100px rgba(0,255,136,.4);}}

        /* ── NEON GREEN OVERRIDES — force full brightness everywhere ── */
        .green-text { color:#00ff88 !important; }
        .green-border { border-color:#00ff88 !important; }
        ...
        .btn-g{
          ...
          box-shadow:0 4px 28px rgba(0,255,136,.5), 0 0 0 1px rgba(0,255,136,.2);
          display:inline-block;text-decoration:none;
          transition:transform .5s cubic-bezier(.22,1,.36,1),box-shadow .5s;
          position:relative;overflow:hidden;
          min-height:44px;
        }
        .btn-g::after{content:'';position:absolute;top:0;left:-100%;width:60%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.4),transparent);transition:left .55s cubic-bezier(.22,1,.36,1);}
        .btn-g:hover{transform:translateY(-3px) scale(1.03);box-shadow:0 16px 50px rgba(0,255,136,.8), 0 0 0 1px rgba(0,255,136,.4);}
        .btn-g:hover::after{left:140%;}
        @media(hover:hover){.btn-g{animation:glowPulse 3s ease-in-out infinite;}}
        .btn-g:hover{animation:none;}

        .btn-ghost{
          background:${dark?"rgba(255,255,255,.06)":"rgba(26,16,0,.08)"};
          color:${dark?"rgba(255,255,255,.7)":"rgba(26,16,0,.82)"};
          border:.5px solid ${dark?"rgba(255,255,255,.15)":"rgba(26,16,0,.25)"};
        ...
        .btn-ghost:hover{background:${dark?"rgba(255,255,255,.1)":"rgba(26,16,0,.14)"};border-color:rgba(0,255,136,.6);color:${dark?"#fff":"#1A1000"};transform:translateY(-2px);box-shadow:0 0 20px rgba(0,255,136,.2);}

        /* ── DIVIDER ── */
        .divider{border:none;border-top:.5px solid ${dark?"rgba(255,255,255,.06)":"rgba(26,16,0,.22)"};}

        /* ── GLASS CARDS ── */
        .glass{
          background:${dark
            ?"linear-gradient(135deg,rgba(0,255,136,.06),rgba(0,255,136,.01),rgba(255,255,255,.02))"
            :"linear-gradient(135deg,rgba(255,255,255,.55),rgba(255,220,100,.15))"};
          border:.5px solid ${dark?"rgba(0,255,136,.18)":"rgba(26,16,0,.18)"};
          border-top:.5px solid ${dark?"rgba(0,255,136,.35)":"rgba(255,255,255,.65)"};
          border-radius:16px;position:relative;overflow:hidden;
          transition:transform .5s cubic-bezier(.22,1,.36,1),box-shadow .5s,border-color .3s;
        }
        .glass::before{content:'';position:absolute;top:0;left:10%;right:10%;height:1px;background:${dark?"linear-gradient(90deg,transparent,rgba(0,255,136,.5),transparent)":"linear-gradient(90deg,transparent,rgba(255,255,255,.8),transparent)"};}
        .glass:hover{transform:translateY(-6px) scale(1.01);box-shadow:0 24px 56px rgba(0,255,136,.2),0 4px 16px rgba(0,0,0,.1);border-color:rgba(0,255,136,.5)!important;}

        /* ── STAT CARDS ── */
        .stat-card{
          background:${dark?"rgba(0,255,136,.04)":"rgba(255,255,255,.4)"};
          border:.5px solid ${dark?"rgba(0,255,136,.15)":"rgba(26,16,0,.18)"};
        ...
        .stat-card:hover{border-color:rgba(0,255,136,.7);transform:translateY(-8px) scale(1.04);box-shadow:0 20px 50px rgba(0,255,136,.25);animation:none;}

        /* ── OFFER CARDS ── */
        .offer-card{
          background:${dark
            ?"linear-gradient(135deg,rgba(0,255,136,.05),rgba(0,255,136,.01))"
            :"linear-gradient(135deg,rgba(255,255,255,.5),rgba(255,220,100,.1))"};
          border:.5px solid ${dark?"rgba(0,255,136,.15)":"rgba(26,16,0,.18)"};
          border-radius:20px;padding:2rem;
          transition:transform .5s cubic-bezier(.22,1,.36,1),border-color .3s,box-shadow .5s;
          position:relative;overflow:hidden;
        }
        .offer-card:hover{transform:translateY(-8px) scale(1.01);box-shadow:0 24px 60px rgba(0,255,136,.15);}
        .offer-card.feat{border-color:rgba(0,255,136,.6)!important;background:linear-gradient(135deg,rgba(0,255,136,.12),rgba(0,230,118,.04))!important;animation:glowPulse 3s ease-in-out infinite;box-shadow:0 0 30px rgba(0,255,136,.2);}
        .offer-card.feat:hover{animation:none;}

        /* ── PARTNER CARDS ── */
        .partner-card{
          background:${dark?"rgba(0,255,136,.04)":"rgba(255,255,255,.4)"};
          border:.5px solid ${dark?"rgba(0,255,136,.14)":"rgba(26,16,0,.15)"};
        ...
        .partner-card:hover{background:rgba(0,255,136,.1)!important;border-color:rgba(0,255,136,.6)!important;transform:translateY(-5px) scale(1.02);box-shadow:0 14px 36px rgba(0,255,136,.2);animation:none;}

        /* ── TESTIMONIAL / REVIEW CARDS ── */
        .testimonial-card{
          border-top:.5px solid rgba(0,255,136,.5) !important;
          box-shadow:0 0 0 .5px rgba(0,255,136,.2);
        }
        .testimonial-card:hover{
          border-color:rgba(0,255,136,.8) !important;
          box-shadow:0 16px 40px rgba(0,255,136,.2) !important;
        }
        /* Force all star ratings to bright gold */
        .star-rating span { color:#FFD700 !important; filter:drop-shadow(0 0 3px rgba(255,215,0,.6)); }
        /* Force all green accent text to full neon */
        ...
        .result-badge{
          ...
          border:.5px solid rgba(0,255,136,.5) !important;
        ...
        .card3d:hover{transform:perspective(900px) rotateY(6deg) rotateX(-4deg) scale(1.03);box-shadow:0 24px 64px rgba(0,0,0,.22),0 0 0 1px rgba(0,255,136,.3);}
