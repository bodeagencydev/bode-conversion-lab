      ...
      {[1,2,3,4,5].map(star => {
        ...
        return (
          <svg key={star} width="14" height="14" viewBox="0 0 24 24">
            <defs><linearGradient id={`half-${star}`}><stop offset="50%" stopColor="#FFD700"/><stop offset="50%" stopColor={dark?"#444":"#ccc"}/></linearGradient></defs>
...
function BrandIcon({ name, slug, color, size = 20 }) {
  ...
  if (failed || !domain) return (
    <div style={{ width:size,height:size,borderRadius:size*.25,background:color?`${color}33`:(dark?"rgba(255,255,255,.15)":"rgba(0,0,0,.1)"),border:`1px solid ${color||(dark?"rgba(255,255,255,.2)":"rgba(0,0,0,.15)")}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center" }}>
      <span style={{ fontSize:size*.5,fontWeight:700,color:color||(dark?"#fff":"#000") }}>{name?.[0]||"?"}</span>
    </div>
  );
  return <img src={`https://www.google.com/s2/favicons?domain=${domain}&sz=64`} alt={name} width={size} height={size} style={{ borderRadius:size*.2,flexShrink:0,display:"block",objectFit:"contain" }} onError={()=>setFailed(true)}/>;
}
...
function VideoCard({ v }) {
  ...
  return (
    <div style={{ flexShrink:0,width:250,scrollSnapAlign:"start",background:dark?"linear-gradient(135deg,rgba(255,255,255,.06),rgba(255,255,255,.02))":"linear-gradient(135deg,rgba(0,0,0,.04),rgba(0,0,0,.01))",border:dark?".5px solid rgba(255,255,255,.12)":".5px solid rgba(0,0,0,.1)",borderTop:dark?".5px solid rgba(255,255,255,.2)":".5px solid rgba(0,0,0,.15)",borderRadius:20,overflow:"hidden",transition:"transform .3s,border-color .3s,box-shadow .3s" }}
      onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-6px) scale(1.01)";e.currentTarget.style.borderColor="rgba(0,255,136,.4)";e.currentTarget.style.boxShadow="0 16px 40px rgba(0,255,136,.1)";}}
      onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.borderColor=dark?"rgba(255,255,255,.12)":"rgba(0,0,0,.1)";e.currentTarget.style.boxShadow="none";}}>
      <div style={{ width:"100%",aspectRatio:"9/16",position:"relative",overflow:"hidden",background:"#000" }}>
        {playing?(
          ...
          <>
            ...
            <div style={{ position:"absolute",inset:0,background:"rgba(0,0,0,.3)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer" }} onClick={()=>setPlaying(true)}>
...
export function VideoTips({ items }) {
  ...
  return (
    <div style={{ position:"relative" }}>
      <button onClick={()=>scroll(-1)} style={{ position:"absolute",left:-16,top:"40%",transform:"translateY(-50%)",zIndex:10,width:36,height:36,borderRadius:"50%",background:"rgba(0,255,136,.15)",border:".5px solid rgba(0,255,136,.4)",color:G,fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"transform .2s" }}
        onMouseEnter={e=>e.currentTarget.style.transform="translateY(-50%) scale(1.1)"}
        onMouseLeave={e=>e.currentTarget.style.transform="translateY(-50%) scale(1)"}>‹</button>
      <button onClick={()=>scroll(1)}  style={{ position:"absolute",right:-16,top:"40%",transform:"translateY(-50%)",zIndex:10,width:36,height:36,borderRadius:"50%",background:"rgba(0,255,136,.15)",border:".5px solid rgba(0,255,136,.4)",color:G,fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"transform .2s" }}
        onMouseEnter={e=>e.currentTarget.style.transform="translateY(-50%) scale(1.1)"}
        onMouseLeave={e=>e.currentTarget.style.transform="translateY(-50%) scale(1)"}>›</button>
    ...
    <div style={{ overflow:"hidden" }} onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
      <div style={{ display:"flex",gap:"1.5rem",width:"max-content",animation:`${reverse?"tickerR":"ticker"} ${speed}s linear infinite`,animationPlayState:paused?"paused":"running",padding:"0.3rem 0" }}>
        {doubled.map((item,i)=>{
          ...
          return (
            <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"0.5rem 1.2rem",background:dark?"rgba(255,255,255,.04)":"rgba(0,0,0,.04)",border:dark?".5px solid rgba(255,255,255,.1)":".5px solid rgba(0,0,0,.1)",borderRadius:100,whiteSpace:"nowrap",cursor:"default",transition:"all .25s" }}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=dark?"rgba(255,255,255,.3)":"rgba(0,0,0,.25)";e.currentTarget.style.background=dark?"rgba(255,255,255,.09)":"rgba(0,0,0,.07)";e.currentTarget.style.transform="translateY(-2px) scale(1.04)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor=dark?"rgba(255,255,255,.1)":"rgba(0,0,0,.1)";e.currentTarget.style.background=dark?"rgba(255,255,255,.04)":"rgba(0,0,0,.04)";e.currentTarget.style.transform="none";}}>
      ...
      <div style={{ display:"flex",gap:"1.5rem",width:"max-content",animation:"ticker 40s linear infinite",animationPlayState:paused?"paused":"running" }}>
        {doubled.map((t,i)=>(
          <a key={i} href={t.storeUrl||"#"} target={t.storeUrl?"_blank":"_self"} rel="noopener noreferrer"
            ...
            onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-6px) scale(1.01)";e.currentTarget.style.borderColor="rgba(0,255,136,.5)";e.currentTarget.style.boxShadow="0 16px 40px rgba(0,255,136,.08)";}}
            onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.borderColor=dark?"rgba(0,255,136,.18)":"rgba(0,180,80,.2)";e.currentTarget.style.boxShadow="none";}}>
...
export function PartnerCard({ partner }) {
  ...
  return (
    <div className="partner-card"
      onMouseEnter={e=>{e.currentTarget.style.background="rgba(0,255,136,.07)";e.currentTarget.style.transform="translateY(-4px) scale(1.02)";e.currentTarget.style.borderColor="rgba(0,255,136,.35)";e.currentTarget.style.boxShadow="0 12px 32px rgba(0,255,136,.1)";}}
  ...
  return (
    <div style={{ display:"flex",alignItems:"center",gap:8,flexShrink:0 }}>
      <img src="/logo.jpeg" alt="Bode Conversion Lab" width={size} height={size}
        style={{ borderRadius:size*.2,objectFit:"cover",flexShrink:0,display:"block" }}/>
      {showText&&(
        <div style={{ display:"flex",flexDirection:"column",lineHeight:1.15 }}>
          ...
          <span style={{ fontWeight:600,fontSize:textSize*.82,fontFamily:"'Syne',sans-serif",color:G,whiteSpace:"nowrap",letterSpacing:".08em",textTransform:"uppercase" }}>Lab</span>
        </div>
      )}
    </div>
  );
...
/* ── THEME TOGGLE — white moon, golden sun ── */
export function ThemeToggle() {
  ...
  return (
    <button onClick={toggle}
      ...
      onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
      {dark ? (
        /* ☀️ Sun — golden, shown in dark mode */
        ...
        /* 🌙 Moon — WHITE, shown in light mode */
...
export function Nav() {
  ...
  const bg = dark?(navH?"rgba(4,6,8,.97)":"rgba(4,6,8,.8)"):(navH?"rgba(248,248,245,.97)":"rgba(248,248,245,.85)");
  return (
    <>
...
/* ── FOOTER — horizontal columns, fully responsive ── */
export function Footer() {
  ...
  const ColLink = ({ to, children }) => (
    <Link to={to} style={{ display:"block",fontSize:14,color:tc,textDecoration:"none",marginBottom:".6rem",transition:"color .2s,transform .15s" }}
      onMouseEnter={e=>{e.target.style.color=G;e.target.style.transform="translateX(3px)";}}
      onMouseLeave={e=>{e.target.style.color=tc;e.target.style.transform="none";}}>{children}</Link>
  );
  ...
  return (
    <footer style={{ padding:"3rem 1.5rem 2rem",borderTop:`.5px solid ${dark?"rgba(255,255,255,.06)":"rgba(0,0,0,.08)"}`,background:dark?"rgba(0,0,0,.3)":"rgba(0,0,0,.04)" }}>
      <style>{`
        .footer-outer { max-width:1100px; margin:0 auto; }
        .footer-grid  {
          display: grid;
      ...
      <div className="footer-outer">
        <div className="footer-grid">
          ...
          <div className="footer-brand">
            ...
            <p style={{ fontSize:13,color:tc,lineHeight:1.7,maxWidth:260,marginTop:6 }}>
            ...
            <a href="https://wa.me/19454076473" target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex",alignItems:"center",gap:6,fontSize:13,color:"#25D366",textDecoration:"none",marginTop:14,transition:"transform .15s" }}
          ...
          {/* Free Tools */}
          <div>
            <p className="footer-col-head">Free Tools</p>
          ...
          <div>
            ...
            <Link to="/contact" style={{ display:"inline-block",background:GG,color:"#040608",borderRadius:8,padding:".65rem 1.2rem",fontSize:13,fontWeight:700,textDecoration:"none",marginBottom:".8rem",transition:"transform .15s,box-shadow .15s" }}
              onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 6px 20px rgba(0,255,136,.4)";}}
              onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="none";}}>Apply Now →</Link>
