    ...
    <PageWrapper>
          ...
          <Link to={`/blog/${BLOG_POSTS[0].id}`} style={{ textDecoration: "none", display: "block", marginBottom: "2rem" }}>
            <div className="glass card3d" style={{ padding: "3rem", cursor: "pointer" }}
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: "1.2rem" }}>
                ...
                <span style={{ background: "rgba(255,255,255,.05)", border: ".5px solid rgba(255,255,255,.1)", borderRadius: 100, padding: "3px 10px", fontSize: 11, color: "rgba(255,255,255,.4)" }}>{BLOG_POSTS[0].category}</span>
              </div>
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.8rem", fontWeight: 800, color: "#fff", marginBottom: "1rem", lineHeight: 1.2 }}>{BLOG_POSTS[0].title}</h2>
              <p style={{ fontSize: 15, color: "rgba(255,255,255,.5)", lineHeight: 1.7, marginBottom: "1.5rem" }}>{BLOG_POSTS[0].excerpt}</p>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", gap: "1.5rem" }}>
                  <span style={{ fontSize: 12, color: "rgba(255,255,255,.3)" }}>{BLOG_POSTS[0].date}</span>
                  <span style={{ fontSize: 12, color: "rgba(255,255,255,.3)" }}>{BLOG_POSTS[0].readTime}</span>
                </div>
            ...
            {BLOG_POSTS.slice(1).map((post) => (
              <Link key={post.id} to={`/blog/${post.id}`} style={{ textDecoration: "none" }}>
                <div className="glass card3d" style={{ padding: "2rem", height: "100%", cursor: "pointer", display: "flex", flexDirection: "column" }}
                  ...
                  <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.05rem", fontWeight: 800, color: "#fff", marginBottom: ".75rem", lineHeight: 1.3, flex: 1 }}>{post.title}</h3>
                  <p style={{ fontSize: 13, color: "rgba(255,255,255,.4)", lineHeight: 1.6, marginBottom: "1.2rem" }}>{post.excerpt.slice(0, 100)}...</p>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,.25)" }}>{post.readTime}</span>
      ...
      <Section style={{ paddingBottom: "8rem" }}>
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 15, color: "rgba(255,255,255,.45)", lineHeight: 1.7, margin: "1.5rem 0" }}>The fastest way to implement what you've read is with an expert who's done it 40+ times.</p>
    ...
    <PageWrapper>
      <section style={{ padding: "6rem 2rem 4rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <Link to="/blog" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,.4)", textDecoration: "none", fontSize: 13, marginBottom: "2rem" }}>← Back to blog</Link>
          <div style={{ display: "flex", gap: 8, marginBottom: "1.2rem" }}>
          ...
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: "2.6rem", fontWeight: 800, lineHeight: 1.15, letterSpacing: "-.02em", color: "#fff", marginBottom: "1rem" }}>{post.title}</h1>
          <div style={{ display: "flex", gap: "1.5rem", marginBottom: "2rem" }}>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,.3)" }}>{post.date}</span>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,.3)" }}>{post.readTime}</span>
          </div>
          <p style={{ fontSize: "1.1rem", color: "rgba(255,255,255,.55)", lineHeight: 1.75, fontStyle: "italic", borderLeft: `2px solid ${G}`, paddingLeft: "1.5rem" }}>{post.excerpt}</p>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
            {post.content.map((section, i) => (
              <div key={i}>
                <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.3rem", fontWeight: 800, color: "#fff", marginBottom: "1rem" }}>{section.heading}</h2>
                <p style={{ fontSize: 15, color: "rgba(255,255,255,.55)", lineHeight: 1.85 }}>{section.body}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "linear-gradient(135deg,rgba(0,255,136,.08),rgba(0,204,106,.03))", border: ".5px solid rgba(0,255,136,.25)", borderRadius: 20, padding: "2.5rem", textAlign: "center", marginTop: "4rem" }}>
            <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.4rem", fontWeight: 800, color: "#fff", marginBottom: ".75rem" }}>Want us to implement this for you?</h3>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,.45)", marginBottom: "1.5rem", lineHeight: 1.7 }}>Apply for a free store audit. We'll identify your biggest opportunities and build the system to capture them.</p>
            <Link to="/contact" className="btn-g" style={{ display: "inline-block" }}>Apply for your free audit →</Link>
          </div>
        </div>
      </Section>
