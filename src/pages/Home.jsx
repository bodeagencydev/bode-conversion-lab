...
import { ScrollReveal, TiltCard, Magnetic, GlowBorder, Counter, SplitText } from "../AnimationSystem.jsx";
    ...
    <PageWrapper>
      ...
      <section style={{ position:"relative", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"clamp(5rem,10vw,8rem) clamp(1rem,4vw,2rem) 4rem", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:780, textAlign:"center", position:"relative", zIndex:1, width:"100%" }}>
          ...
          <h1 className="hero-t" style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(2rem,5vw,3.7rem)", fontWeight:800, lineHeight:1.07, letterSpacing:"-.03em", marginBottom:"1.2rem", color:headingColor, animation:"heroFadeUp .8s .1s ease both", animationFillMode:"forwards", opacity:0, wordBreak:"break-word", marginTop:"1.2rem" }}>
          ...
          <ScrollReveal delay={0.3}>
            <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:"3.5rem" }}>
              {/* ✅ FIXED — now leads to /audit */}
              <Magnetic><Link to="/audit" className="btn-g" data-cursor="Audit">See if your store qualifies →</Link></Magnetic>
              <Magnetic><Link to="/case-studies" className="btn-ghost">View client results</Link></Magnetic>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal delay={0.4}>
            <div className="hero-cards" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
              {[{ n:"70x", l:"Revenue growth\nin 90 days" }, { n:"4x+", l:"Average ROAS\nimprovement" }, { n:"$0", l:"Extra ad spend\nrequired" }].map((c,i) => (
                <TiltCard key={i} style={{ background:"linear-gradient(135deg,rgba(0,255,136,.09),rgba(0,204,106,.03))", border:".5px solid rgba(0,255,136,.22)", borderTop:".5px solid rgba(0,255,136,.38)", borderRadius:16, padding:"1.3rem 1rem", textAlign:"center", position:"relative", overflow:"hidden" }}>
              ...
              <ScrollReveal key={i} delay={i * 0.12}>
                <div className="stat-card">
                  <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(2rem,5vw,3rem)", fontWeight:800, background:GG, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", lineHeight:1, marginBottom:8 }}>
                    {statsInView ? <Counter to={s.n} suffix={s.s} duration={2000} /> : `0${s.s}`}
                  </div>
        ...
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <ScrollReveal delay={0.1}>
            <div style={{ textAlign:"center", marginTop:"2.5rem" }}>
              <Magnetic><Link to="/case-studies" className="btn-ghost">Read full case studies →</Link></Magnetic>
            </div>
          </ScrollReveal>
        </div>
      </Section>
      ...
      <Section>
        <ScrollReveal delay={0}>
          <div style={{ maxWidth:760, margin:"0 auto", textAlign:"center" }}>
            <GlowBorder style={{ background:"linear-gradient(135deg,rgba(0,255,136,.08),rgba(0,204,106,.03))", border:".5px solid rgba(0,255,136,.25)", borderRadius:24, padding:"clamp(2.5rem,5vw,5rem) clamp(1.5rem,4vw,3rem)", overflow:"hidden" }}>
