          ...
          {CASE_STUDIES.map((cs, i) => (
            <Link key={cs.id} to={`/case-studies/${cs.id}`} style={{ textDecoration: "none", display: "block" }}>
              <div className="glass" style={{ padding: "clamp(1.2rem,4vw,2.5rem)", cursor: "pointer", transition: "border-color .3s" }}
                ...
                {/* Headline */}
                <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(1.1rem,4vw,1.5rem)", fontWeight: 800, color: "#fff", marginBottom: ".75rem", lineHeight: 1.2, wordBreak: "break-word" }}>{cs.headline}</h2>
                <p style={{ fontSize: "clamp(13px,3vw,14px)", color: "rgba(255,255,255,.45)", lineHeight: 1.7, marginBottom: "1.5rem" }}>{cs.summary}</p>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.75rem", marginBottom: "1.2rem" }}>
                  {[cs.result1, cs.result2, cs.result3].map((r, j) => (
                    <div key={j} style={{ textAlign: "center", background: "rgba(0,255,136,.06)", border: ".5px solid rgba(0,255,136,.18)", borderRadius: 12, padding: "clamp(0.6rem,2vw,1rem) clamp(0.5rem,2vw,1.2rem)" }}>
                      <p style={{ fontSize: "clamp(9px,2.5vw,11px)", color: "rgba(255,255,255,.3)", marginBottom: 4 }}>{r.label}</p>
                      <p style={{ fontSize: "clamp(10px,2.5vw,13px)", color: "rgba(255,255,255,.4)", marginBottom: 2 }}>{r.before}</p>
    ...
    <PageWrapper>
      <section style={{ position: "relative", padding: "clamp(4rem,8vw,6rem) clamp(1rem,4vw,2rem) 3rem", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(1.6rem,5vw,2.8rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-.02em", color: "#fff", marginBottom: "1rem", wordBreak: "break-word" }}>{cs.headline}</h1>
          <p style={{ fontSize: "clamp(0.9rem,3vw,1.05rem)", color: "rgba(255,255,255,.45)", lineHeight: 1.75 }}>{cs.summary}</p>
        </div>
      </section>
      ...
      <Section>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1rem", marginBottom: "4rem" }}>
            {[cs.result1, cs.result2, cs.result3].map((r, i) => (
              <div key={i} className="stat-card" style={{ textAlign: "center" }}>
                ...
                <p style={{ fontSize: "clamp(11px,3vw,14px)", color: "rgba(255,255,255,.4)", marginBottom: 4 }}>Before: <span style={{ color: "#ff6b6b" }}>{r.before}</span></p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
            {cs.story.map((s, i) => (
              <div key={i} style={{ borderLeft: `2px solid rgba(0,255,136,0.3)`, paddingLeft: "clamp(1rem,4vw,2rem)" }}>
                <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(1rem,4vw,1.2rem)", fontWeight: 800, color: "#fff", marginBottom: "1rem" }}>{s.heading}</h3>
                <p style={{ fontSize: "clamp(13px,3vw,15px)", color: "rgba(255,255,255,.5)", lineHeight: 1.85 }}>{s.body}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "linear-gradient(135deg,rgba(0,255,136,.08),rgba(0,204,106,.03))", border: ".5px solid rgba(0,255,136,.25)", borderRadius: 20, padding: "clamp(1.5rem,4vw,2.5rem)", textAlign: "center", marginTop: "4rem" }}>
            <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(1.2rem,4vw,1.5rem)", fontWeight: 800, color: "#fff", marginBottom: ".75rem" }}>Want results like these?</h3>
