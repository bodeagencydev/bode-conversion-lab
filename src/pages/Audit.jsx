...
/* ─── CLIENT SNAPSHOT CARD — HTML/CSS version, captured to PNG via
    ...
    <div ref={innerRef} style={{ width:860, background:"#fff", fontFamily:"'IBM Plex Sans',sans-serif", color:"#1A1408" }}>
      ...
      <div style={{ padding:"28px 40px" }}>
        <div style={{ display:"flex", gap:16 }}>
          <div style={{ flex:1, border:"1px solid #EEE7D8", borderRadius:14, padding:20 }}>
            <p style={{ fontSize:10.5, fontWeight:800, color:"#FF3B3B", textTransform:"uppercase", letterSpacing:.8, marginBottom:10 }}>The Bottom Line</p>
            <p style={{ fontSize:11.5, color:"#6B6249", lineHeight:1.5, marginBottom:6 }}>Based on current traffic and conversion leaks, you're losing an estimated:</p>
            <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:38, fontWeight:800, color:"#FF3B3B", margin:0 }}>{fmt(lossLow)} – {fmt(lossHigh)}</p>
            <p style={{ fontSize:14, fontWeight:800, color:"#1A1408", margin:"2px 0 10px" }}>PER MONTH</p>
            <p style={{ fontSize:10.5, color:"#8C8465", lineHeight:1.5 }}>That's {fmt(lossLow*12)} – {fmt(lossHigh*12)} per year in potential revenue walking away.</p>
          </div>
          <div style={{ flex:1, border:"1px solid #EEE7D8", borderRadius:14, padding:20 }}>
            <p style={{ fontSize:10.5, fontWeight:800, color:"#1A1408", textTransform:"uppercase", letterSpacing:.8, marginBottom:10 }}>How We Calculated This</p>
            {[
              ["Estimated Monthly Visitors", visitors.toLocaleString()],
              ["Current Conversion Rate (est.)", currentCR+"%"],
              ["Potential Conversion Rate (fixed)", `${potentialCRLow}% – ${potentialCRHigh}%`],
              ["Average Order Value", "$"+aov],
            ].map(([l,v],i) => (
              <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"7px 0", borderBottom:"1px solid #EEE7D8" }}>
                <span style={{ fontSize:11, color:"#6B6249" }}>{l}</span>
                <span style={{ fontSize:11, fontWeight:700, color:"#1A1408" }}>{v}</span>
              </div>
            ))}
            <div style={{ display:"flex", justifyContent:"space-between", padding:"8px 0 0" }}>
              <span style={{ fontSize:11, fontWeight:800, color:"#FF3B3B" }}>Revenue Lost Per Month</span>
              <span style={{ fontSize:11, fontWeight:800, color:"#FF3B3B" }}>{fmt(lossLow)} – {fmt(lossHigh)}</span>
            </div>
          </div>
        </div>

        <p style={{ fontSize:15, fontWeight:800, color:"#1A1408", margin:"24px 0 12px" }}>Where The Money Is Leaking</p>
        <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
          {leaks.map((l,i) => (
            <div key={i} style={{ width:"calc(33.33% - 7px)", border:"1px solid #EEE7D8", borderRadius:12, padding:14 }}>
              <div style={{ width:10, height:10, borderRadius:5, background:"#FF3B3B", marginBottom:8 }} />
              <p style={{ fontSize:11.5, fontWeight:800, color:"#1A1408", margin:"0 0 6px", lineHeight:1.3 }}>{l.title}</p>
              <p style={{ fontSize:10, color:"#8C8465", lineHeight:1.45, margin:"0 0 8px", minHeight:44 }}>{l.desc}</p>
              <p style={{ fontSize:11, fontWeight:800, color:"#FF3B3B", margin:0 }}>{fmt(l.lossLow)} – {fmt(l.lossHigh)}</p>
            </div>
          ))}
        </div>

        <div style={{ display:"flex", gap:16, marginTop:20 }}>
          <div style={{ flex:1, background:"#0A0F0C", borderRadius:14, padding:20 }}>
            <p style={{ fontSize:11, fontWeight:800, color:"#fff", marginBottom:14 }}>Current Store Health</p>
            <div style={{ display:"flex", gap:16, alignItems:"center" }}>
              <div style={{ position:"relative", width:90, height:90, flexShrink:0 }}>
                <Ring score={overall} size={90} color={overall>=70?"#00FF88":overall>=45?"#FFD700":"#FF3B3B"} />
                <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
                  <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:22, fontWeight:800, color:"#fff" }}>{overall}</span>
                  <span style={{ fontSize:9, color:"#8C8C8C" }}>{grade}</span>
                </div>
              </div>
              <div style={{ flex:1 }}>
                <p style={{ fontSize:10, color:"#B8B8B8", marginBottom:6 }}>{problemCount} problems identified</p>
                {criticalCount > 0 && (
                  <span style={{ display:"inline-block", background:"#FF3B3B", borderRadius:4, padding:"2px 8px", fontSize:9, fontWeight:800, color:"#fff", marginBottom:8 }}>{criticalCount} CRITICAL ISSUES</span>
                )}
                {topIssues.slice(0,4).map((t,i) => (
                  <p key={i} style={{ fontSize:9.5, color:"#DDDDDD", margin:"3px 0", lineHeight:1.4 }}>• {t}</p>
                ))}
              </div>
            </div>
          </div>
          <div style={{ flex:1, border:"1px solid #EEE7D8", borderRadius:14, padding:20 }}>
            <p style={{ fontSize:11, fontWeight:800, color:"#1A1408", marginBottom:12 }}>What This Means For Your Business</p>
            {[
              ...
              <p key={i} style={{ fontSize:10.5, color:"#6B6249", lineHeight:1.5, margin:"0 0 8px" }}>• {t}</p>
            ))}
          </div>
        </div>
      ...
      <div style={{ background:"#0A0F0C", padding:"16px 40px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <p style={{ fontSize:12, fontWeight:800, color:"#fff", margin:0 }}>bodeconversionlab.vercel.app</p>
          <p style={{ fontSize:9.5, color:"#8C8C8C", margin:"2px 0 0" }}>We fix the leaks. You keep the profit.</p>
        </div>
        <div style={{ background:"#00FF88", borderRadius:8, padding:"9px 16px" }}>
          <p style={{ fontSize:10.5, fontWeight:800, color:"#0A0F0C", margin:0 }}>calendly.com/bodeagencyofficial/30min</p>
        </div>
      </div>
    </div>
  );
...
export default function Audit() {
  ...
  async function handleSnapshotDownload() {
    ...
    try {
      ...
      setSnapReady(props);
      // Wait a beat for the hidden card (and its logo image) to actually
      // paint before html2canvas reads the DOM — capturing on the same
      // tick sometimes grabs the card before layout settles.
      await new Promise(r => setTimeout(r, 150));
      const { default: html2canvas } = await import("html2canvas");
      const canvas = await html2canvas(snapshotRef.current, { backgroundColor: "#ffffff", scale: 2, useCORS: true });
      const a = document.createElement("a");
      a.href = canvas.toDataURL("image/png");
      a.download = `BCL-Snapshot-${domain}.png`;
      a.click();
    } catch (err) {
        ...
        <div style={{ maxWidth:980, margin:"0 auto", padding:"clamp(2rem,5vw,4rem) clamp(1rem,4vw,2rem) 6rem", opacity:revealed?1:0, transition:"opacity .6s ease" }}>
          ...
          {analysis.isCritical && (
            <div style={{ background:"linear-gradient(135deg,rgba(255,59,59,.12),rgba(255,59,59,.04))", border:"1px solid rgba(255,59,59,.4)", borderRadius:16, padding:"1rem 1.4rem", marginBottom:"1.5rem", display:"flex", alignItems:"center", gap:12, animation:"criticalPulse 2.5s ease-in-out infinite" }}>
              ...
              <div>
                <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:14, fontWeight:800, color:"#FF3B3B", marginBottom:2 }}>CRITICAL REVENUE LEAK DETECTED</p>
          ...
          <div style={{ background:cardBg, border:`.5px solid ${analysis.isCritical?"rgba(255,59,59,.35)":cardBorder}`, borderRadius:24, padding:"1.8rem 2rem", marginBottom:"1.5rem", position:"relative", overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", gap:"1.5rem", alignItems:"center", flexWrap:"wrap" }}>
              ...
              <div style={{ flex:1, minWidth:200 }}>
                <h2 style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:"clamp(1rem,3vw,1.4rem)", fontWeight:800, color:headingColor, marginBottom:".3rem" }}>{analysis.domain}</h2>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:"0.6rem" }} className="stat-grid">
              {Object.values(analysis.vitals).map((v,i) => (
                <div key={i} style={{ textAlign:"center", background:dark?"rgba(255,255,255,.03)":"rgba(255,255,255,.35)", border:`.5px solid ${vitC(v.status)}44`, borderRadius:10, padding:".75rem .5rem" }}>
                  ...
                  <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:"1rem", fontWeight:800, color:vitC(v.status) }}>{v.value}</p>
            ...
            {["critical","high","medium"].map(sev => {
              ...
              return (
                <div key={sev} style={{ background:`${sevColor(sev)}18`, border:`.5px solid ${sevColor(sev)}44`, borderRadius:10, padding:".6rem 1rem", display:"flex", alignItems:"center", gap:8 }}>
                  <span style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:"1.2rem", fontWeight:800, color:sevColor(sev) }}>{count}</span>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:"0.9rem" }}>
              {[
                ...
                <div key={i} style={{ background:dark?"rgba(255,255,255,.04)":"rgba(255,255,255,.5)", border:".5px solid rgba(0,255,136,.18)", borderRadius:14, padding:"1.1rem 1.2rem", position:"relative" }}>
                  ...
                  <h4 style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:14, fontWeight:800, color:headingColor, marginBottom:".4rem" }}>{p.title}</h4>
          ...
          {snapReady && (
            <div style={{ position:"fixed", top:0, left:-99999, pointerEvents:"none" }}>
              <ClientSnapshotCard innerRef={snapshotRef} {...snapReady} />
            </div>
          )}
          ...
          <div style={{ position:"relative", marginBottom:"2rem" }}>
            <div style={{ background:cardBg, border:`.5px solid ${cardBorder}`, borderRadius:20, padding:"1.8rem", filter:accessTier?"none":"blur(5px)", pointerEvents:accessTier?"auto":"none", userSelect:accessTier?"auto":"none" }}>
              <h3 style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:"1.1rem", fontWeight:800, color:headingColor, marginBottom:"1rem" }}>SRS — Phase 1 Preview</h3>
