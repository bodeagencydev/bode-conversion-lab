    ...
    <PageWrapper>
      <section style={{ position: "relative", minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "clamp(4rem,8vw,6rem) clamp(1rem,4vw,2rem) 3rem", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 760, textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <p style={{ fontSize: "clamp(0.9rem,3vw,1.05rem)", color: "rgba(255,255,255,.45)", lineHeight: 1.75, maxWidth: 520, margin: "0 auto" }}>
      ...
      <Section>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(2rem,5vw,5rem)", alignItems: "center" }} className="about-grid">
          <div>
            ...
            <p style={{ fontSize: 15, color: "rgba(255,255,255,.5)", lineHeight: 1.85, marginTop: "1.5rem" }}>Bode Conversion Lab was born out of frustration. After seeing store after store burn money on ads while the real problems went untouched, we built a different kind of service.</p>
            <p style={{ fontSize: 15, color: "rgba(255,255,255,.5)", lineHeight: 1.85, marginTop: "1rem" }}>We don't start with ads. We start with an audit. We find every single place your funnel is leaking money. Then we fix it. Then we scale it.</p>
          </div>
      ...
      <Section>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,.4)", marginTop: ".75rem" }}>Tap any service to see what's included</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "1rem" }}>
            {SERVICES.map((s) => (
              <div key={s.id}
                onClick={() => setActiveService(activeService === s.id ? null : s.id)}
                style={{ background: activeService === s.id ? "linear-gradient(135deg,rgba(0,255,136,.1),rgba(0,204,106,.04))" : "linear-gradient(135deg,rgba(255,255,255,.06),rgba(255,255,255,.02))", border: activeService === s.id ? `.5px solid rgba(0,255,136,.45)` : ".5px solid rgba(255,255,255,.12)", borderRadius: 16, padding: "1.5rem", cursor: "pointer", transition: "all .3s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
                  <div>
                    <span style={{ fontSize: 24, display: "block", marginBottom: ".5rem" }}>{s.icon}</span>
                    <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.1rem", fontWeight: 800, color: "#fff", margin: 0 }}>{s.title}</h3>
                ...
                <p style={{ fontSize: 13, color: "rgba(255,255,255,.45)", lineHeight: 1.7, marginBottom: activeService === s.id ? "1rem" : 0 }}>{s.desc}</p>
                {activeService === s.id && (
                  <div>
                    <ul style={{ listStyle: "none", marginBottom: "1.2rem" }}>
                      {s.bullets.map((b, j) => (
                        <li key={j} style={{ fontSize: 13, color: "rgba(255,255,255,.6)", padding: "6px 0", borderBottom: ".5px solid rgba(255,255,255,.06)", display: "flex", gap: 8, alignItems: "center" }}>
